...
class XLMRobertaEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, language_ids=None, inputs_embeds=None, past_key_values_length=0
    ):
        ...
        embeddings = inputs_embeds + token_type_embeddings + language_embeddings
...
class XLMRobertaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([XLMRobertaLayer(config)
...
class XLMRobertaForMultipleChoice(XLMRobertaPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(
        XLM_ROBERTA_INPUTS_DOCSTRING.format(
